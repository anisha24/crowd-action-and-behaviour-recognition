...
def dense_flow(augs):
    ...
    while True:
        ...
        # Perform optical flow computation using NVIDIA Optical Flow SDK
        flowDTVL1 = compute_optical_flow(frame_0, frame_1)  # You need to implement this function
...
def compute_optical_flow(prev_frame, next_frame):
    """
    Computes optical flow between two frames using NVIDIA Optical Flow SDK.
    """
    # You need to implement this function using NVIDIA Optical Flow SDK
    # This function should return the computed optical flow between prev_frame and next_frame
    # Below is a dummy implementation using DALI library, replace it with NVIDIA Optical Flow SDK
    prev_frame = np.expand_dims(prev_frame, axis=0)
    next_frame = np.expand_dims(next_frame, axis=0)
    optical_flow = ops.OpticalFlow()
    flow = optical_flow(
        prev_frame=ops.ExternalSource(),
        next_frame=ops.ExternalSource()
    )
    with ops.Device(device="gpu", id=0):
        pipe = ops.Pipeline(batch_size=1, num_threads=1, device_id=0)
        with pipe:
            prev = ops.ExternalSource()
            next = ops.ExternalSource()
            flow = optical_flow(prev, next)
            pipe.set_outputs(prev, next, flow)
        pipe.build()
        outputs = pipe.run(inputs={prev: prev_frame, next: next_frame})
    return outputs[2][0]
